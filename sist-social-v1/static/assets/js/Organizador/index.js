$(function(){
    ...
    tabela = $('#datatable').DataTable({
        ...
        'ajax': {
            'url': caminhoBase+nomeController+'/dataTable',
            'type': 'POST'
        },
...
function deletarOrganizador(id)
{
    metodo = 'DELETE';
    swal({
...
function criarBotoes(ids){
    $.ajax({
        type: 'GET',
        dataType:'JSON',
        url: 'Organizador/botoesAcesso',
    }).done(function(retorno) {
        for(var i = 0; i < ids.length; i++){
            var botoes = '';
            if (retorno.btnEditar) {
                botoes += '<a href="Organizador/editar/' + ids[i] + '" class="btn btn-default btn-sm">' +
